...
const ROW_COLORS = ['#2DD4BF', '#F87171', '#94A3B8', '#FBBF24'];

export default function WorkedExampleInjectionCaught() {
  const [example, setExample] = useState<LectureExample | null>(null);
  const [failed, setFailed] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetchLectureExample(17)
      .then(data => {
    ...
    <div style={wrap} dir={dir}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '1vw' }}>
          ...
          <span>&bull;</span>
          <span>{t('Slide 17 of 21', 'שקופית 17 מתוך 21')}</span>
        </div>
      </div>
    </div>
  );
