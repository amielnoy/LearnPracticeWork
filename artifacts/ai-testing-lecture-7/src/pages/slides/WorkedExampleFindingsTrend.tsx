        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Aggregate open findings count per week to visualise security debt', 'צבור ספירת ממצאים פתוחים לכל שבוע לתצוגה של חוב אבטחה')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', minWidth: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', marginTop: '0.7vw' }} />
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Alert when the open findings backlog grows week-over-week', 'הצג התראה כאשר מצבור הממצאים הפתוחים גדל שבוע אחרי שבוע')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', minWidth: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', marginTop: '0.7vw' }} />
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Include MTTR (mean time to remediate) to track the team\'s response velocity', 'כלול MTTR (זמן ממוצע לתיקון) למעקב אחר מהירות תגובת הצוות')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codePanel}>
          ...
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            {'const { data: weekly } = await supabase'}<br />
            {'  .from("security_findings")'}<br />
            {'  .select(`'}<br />
            {'    remediation_status,'}<br />
            {'    severity,'}<br />
            {'    created_at,'}<br />
            {'    resolved_at`)'}<br />
          ...
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            {'// Compute MTTR for resolved findings'}<br />
            {'const resolved = weekly.filter('}<br />
            {'  f => f.remediation_status === "resolved"'}<br />
            {');'}<br />
            {'const mttr = resolved.reduce((sum, f) => {'}<br />
            {'  return sum + (new Date(f.resolved_at)'}<br />
            {'    - new Date(f.created_at));'}<br />
            {'}, 0) / resolved.length / 3600000;'}<br />
            {'// mttr in hours'}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
            <span style={{ background: '#D97706', color: '#fff', borderRadius: '0.4vw', padding: '0.5vh 1vw', fontSize: '0.95vw', fontWeight: 700, letterSpacing: '0.05em' }}>
              MTTR: 18h
            </span>
