        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Fetch adversarial test results grouped by build date', 'שלוף תוצאות בדיקות עוינות מקובצות לפי תאריך build')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', minWidth: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', marginTop: '0.7vw' }} />
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Compare evasion rate to the baseline from the prior build', 'השווה שיעור התחמקות לבסיס מה-build הקודם')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', minWidth: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', marginTop: '0.7vw' }} />
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Fail the build if evasion rate increases by more than 5%', 'עצור את ה-build אם שיעור ההתחמקות עולה ביותר מ-5%')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codePanel}>
          ...
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            ...
            {'  .from("adversarial_test_runs")'}<br />
            {'  .select("build_sha, evasion_rate, test_date")'}<br />
            {'  .order("test_date", { ascending: false })'}<br />
            {'  .limit(2);'}
          </div>
          <div style={{ height: '1px', background: '#1E293B' }} />
          ...
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            {'const [current, baseline] = data;'}<br />
            {'const delta = current.evasion_rate'}<br />
            {'  - baseline.evasion_rate;'}<br />
            {'if (delta > 5) {'}<br />
            {'  console.error(`Evasion regression: +${delta}%`);'}<br />
            {'  process.exit(1);'}<br />
            {'}'}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
