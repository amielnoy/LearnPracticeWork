...
export default function WorkedExampleRateLimitAudit() {
  return (
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <h1 style={{ fontSize: '2.4vw', fontWeight: 800, margin: '0 0 3vh 0', lineHeight: 1.15, letterSpacing: '-0.02em' }}>
          {t('Rate-Limit Abuse Audit via Supabase', 'ביקורת התעללות ב-Rate-Limit דרך Supabase')}
        </h1>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Log each request with user identity and timestamp to detect burst patterns', 'תעד כל בקשה עם זהות משתמש וחותמת זמן לזיהוי תבניות פרצים')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', minWidth: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', marginTop: '0.7vw' }} />
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Query for users who exceed 5x normal rate to flag abuse candidates', 'שאל על משתמשים שחורגים פי 5 מהקצב הרגיל לסימון מועמדים לניצול לרעה')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', minWidth: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', marginTop: '0.7vw' }} />
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t('Assert throttling test: burst of 50 rps must trigger 429 by request 11', 'ודא בדיקת throttling: פרץ של 50 bps חייב להפעיל 429 עד בקשה 11')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codePanel}>
          <div style={{ fontSize: '0.85vw', fontWeight: 700, color: '#38BDF8', letterSpacing: '0.08em' }}>
            TABLE: rate_limit_test_runs
          </div>
          <div style={{ fontSize: '1.05vw', color: '#94A3B8', lineHeight: 1.5 }}>
            {'user_id | request_count | window_s | got_429 | first_429_at'}
          </div>
          <div style={{ height: '1px', background: '#1E293B' }} />
          <div style={{ fontSize: '0.85vw', fontWeight: 700, color: '#FBBF24', letterSpacing: '0.08em' }}>
            RECORD burst test result
          </div>
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            {'await supabase'}<br />
            {'  .from("rate_limit_test_runs")'}<br />
            {'  .insert({'}<br />
            {'    user_id: testUserId,'}<br />
            {'    request_count: 50,'}<br />
            {'    window_s: 10,'}<br />
            {'    got_429: true,'}<br />
            {'    first_429_at: 11'}<br />
          ...
          <div style={{ height: '1px', background: '#1E293B' }} />
          <div style={{ fontSize: '0.85vw', fontWeight: 700, color: '#2DD4BF', letterSpacing: '0.08em' }}>
            ASSERT throttling triggers on time
          </div>
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.6 }}>
            {'const { data } = await supabase'}<br />
            {'  .from("rate_limit_test_runs")'}<br />
            {'  .select("got_429, first_429_at")'}<br />
            {'  .eq("user_id", testUserId)'}<br />
            {'  .single();'}<br />
            {'assert(data.got_429 === true);'}<br />
            {'assert(data.first_429_at <= 15);'}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
            ...
            <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>
              {t('429 triggered at request 11 of 50', '429 הופעל בבקשה 11 מתוך 50')}
            </span>
          </div>
        </div>
      </div>
