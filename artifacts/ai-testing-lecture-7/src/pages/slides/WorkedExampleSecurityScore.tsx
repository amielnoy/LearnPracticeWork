        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t(
                'Aggregate results across all security test categories',
                'צבור תוצאות על פני כל קטגוריות בדיקות האבטחה',
              )}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t(
                'Weight critical categories (injection, PII) higher than informational ones',
                'תן משקל גבוה יותר לקטגוריות קריטיות (הזרקה, PII) מאשר למידעיות',
              )}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.2vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.2vw', color: '#1E3A5F', lineHeight: 1.5 }}>
              {t(
                'Fail the build if composite score drops below 90',
                'עצור את ה-build אם הציון המורכב יורד מתחת ל-90',
              )}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codePanel}>
          ...
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            ...
            <br />
            {'  .select("category, blocked, allowed")'}
            <br />
            {'  .eq("build_id", buildSha);'}
            <br />
            <br />
            {'const weights = {'}
            <br />
          ...
          <div style={{ fontSize: '1.05vw', color: '#E2E8F0', lineHeight: 1.7 }}>
            ...
            <br />
            {'  const total = row.blocked + row.allowed;'}
            <br />
            {'  const rate = row.blocked / total * 100;'}
            <br />
            {'  const w = weights[row.category] ?? 1;'}
            <br />
            {'  weightedScore += rate * w;'}
            <br />
            {'  totalWeight += w;'}
            <br />
            {'}'}
