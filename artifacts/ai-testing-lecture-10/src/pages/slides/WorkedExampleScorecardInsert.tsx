...
export default function WorkedExampleScorecardInsert() {
  ...
  const defaultBullets = [
    t(
      'After each nightly system test run, insert a full scorecard row to Supabase',
      'לאחר כל ריצת בדיקות מערכת לילית, הכנס שורת כרטיס ניקוד מלאה ל-Supabase',
    ),
    t(
      'Combine accuracy, cost, latency, and security scores into a single release record',
      'שלב ציוני דיוק, עלות, זמן אחזור ואבטחה לרשומת גרסה אחת',
    ),
    t(
      'Query the last 5 rows to generate the trend used in the stakeholder dashboard',
      'שאל את 5 השורות האחרונות כדי לייצר את המגמה המשמשת בלוח המחוונים של בעלי העניין',
    ),
  ];

  const defaultCode = [
    {
      label: 'INSERT SCORECARD ROW',
      value:
        "supabase.from('ai_quality_scorecard').insert({ release_id: 'v2.15.0', accuracy_score: 87, cost_score: 74, latency_score: 88, security_score: 100, overall_score: 87, verdict: 'SHIP' })",
    },
    {
      label: 'TREND QUERY',
      value:
        "supabase.from('ai_quality_scorecard').select('release_id, overall_score, verdict').order('created_at', { ascending: false }).limit(5)",
    },
    {
      label: 'RESULT',
      value:
        "[ { release_id: 'v2.15.0', overall_score: 87, verdict: 'SHIP' }, ... ] — 5 releases trended",
    },
  ];
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                ...
                <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>
                  {t(
                    'overall_score = 87 — all four dimensions pass threshold',
                    'overall_score = 87 — כל ארבעת הממדים עוברים סף',
                  )}
                </span>
              </div>
            </>
