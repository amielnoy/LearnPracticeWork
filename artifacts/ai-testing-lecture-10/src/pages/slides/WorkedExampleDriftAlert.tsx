...
export default function WorkedExampleDriftAlert() {
  ...
  const defaultBullets = [
    t('Write a production drift event to Supabase whenever shadow eval scores drop below threshold', 'כתוב אירוע סחף ייצור ל-Supabase בכל פעם שציוני הערכת הצל יורדים מתחת לסף'),
    t('Query the open drift alerts to surface the most recent incidents on the engineering dashboard', 'שאל את התראות הסחף הפתוחות כדי לגלות את האירועים האחרונים בלוח המחוונים ההנדסי'),
    t('Close the alert and record the remediation action taken when the issue is resolved', 'סגור את ההתראה ותעד את פעולת התיקון שננקטה כאשר הבעיה נפתרת'),
  ];

  const defaultCode = [
    { label: 'INSERT DRIFT ALERT', value: "supabase.from('production_drift_alerts').insert({ feature: 'summarise', shadow_score: 0.74, baseline_score: 0.88, threshold: 0.80, status: 'open', detected_at: new Date().toISOString() })" },
    { label: 'OPEN ALERTS QUERY', value: "supabase.from('production_drift_alerts').select('feature, shadow_score, baseline_score, detected_at').eq('status', 'open').order('detected_at', { ascending: false })" },
    { label: 'CLOSE ALERT', value: "supabase.from('production_drift_alerts').update({ status: 'resolved', resolved_at: new Date().toISOString(), remediation: 'Rolled back prompt v3 to v2' }).eq('id', 7)" },
  ];
        ...
        <div style={codePanel}>
          {failed ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                <span style={{ background: '#DC2626', color: '#fff', borderRadius: '0.4vw', padding: '0.5vh 1vw', fontSize: '0.95vw', fontWeight: 700, letterSpacing: '0.05em' }}>ALERT</span>
                <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>{t('summarise feature drifted below 0.80 threshold — alert created', 'תכונת summarise נסחפה מתחת לסף 0.80 — התראה נוצרה')}</span>
              </div>
            </>
          ) : (
