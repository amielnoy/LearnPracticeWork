...
export default function WorkedExamplePerformanceBenchmarks() {
  ...
  const defaultBullets = [
    t(
      'Write latency benchmark results to Supabase after each nightly run',
      'כתוב תוצאות בסיס זמן אחזור ל-Supabase לאחר כל ריצה לילית',
    ),
    t(
      'Query the last 10 runs to compute rolling p95 and detect regressions',
      'שאל את 10 הריצות האחרונות כדי לחשב p95 נגלל ולזהות רגרסיות',
    ),
  ...
  const defaultCode = [
    {
      label: 'INSERT BENCHMARK',
      value:
        "supabase.from('latency_benchmarks').insert({ release_id: 'v2.15.0', run_date: '2025-11-15', p50_ms: 1820, p95_ms: 3650, p99_ms: 6200, ttft_ms: 540 })",
    },
    {
      label: 'REGRESSION QUERY',
      value:
        "supabase.from('latency_benchmarks').select('run_date, p95_ms').order('run_date', { ascending: false }).limit(10)",
    },
    {
      label: 'THRESHOLD CHECK',
      value: 'data.filter(row => row.p95_ms > 4000) // Alert if any recent run exceeds SLA',
    },
  ];
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                ...
                <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>
                  {t('p95 = 3650ms — within 4s SLA', 'p95 = 3650ms — בתוך SLA של 4 שניות')}
                </span>
              </div>
            </>
          ) : (
