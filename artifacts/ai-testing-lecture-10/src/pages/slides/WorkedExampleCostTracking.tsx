...
export default function WorkedExampleCostTracking() {
  ...
  const defaultBullets = [
    t('Track cost per feature invocation in Supabase across all releases', 'עקוב אחר עלות לכל הפעלת תכונה ב-Supabase בכל הגרסאות'),
    t('Compute the 30-day rolling average and compare to the approved budget', 'חשב את הממוצע הנגלל של 30 יום והשווה לתקציב המאושר'),
    t('Flag releases where cost per invocation exceeds the budget by more than 10%', 'סמן גרסאות שבהן עלות להפעלה עולה על התקציב ביותר מ-10%'),
  ];

  const defaultCode = [
    { label: 'INSERT COST RUN', value: "supabase.from('ai_cost_runs').insert({ release_id: 'v2.15.0', feature: 'chat_assist', input_tokens: 1280, output_tokens: 340, cost_usd: 0.0048, budget_usd: 0.004 })" },
    { label: 'OVER-BUDGET QUERY', value: "supabase.from('ai_cost_runs').select('release_id, feature, cost_usd, budget_usd').filter('cost_usd', 'gt', 'budget_usd * 1.10').order('cost_usd', { ascending: false })" },
    { label: 'RESULT', value: "[ { release_id: 'v2.15.0', feature: 'chat_assist', cost_usd: 0.0048, budget_usd: 0.004 } ] — 20% over budget" },
  ];
    ...
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <h1 style={{ fontSize: '2.4vw', fontWeight: 800, margin: '0 0 3vh 0', lineHeight: 1.15, letterSpacing: '-0.02em' }}>
          {example?.title ?? t('Cost Tracking Per Feature', 'מעקב עלות לכל תכונה')}
        </h1>
        ...
        <div style={codePanel}>
          {failed ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                <span style={{ background: '#D97706', color: '#fff', borderRadius: '0.4vw', padding: '0.5vh 1vw', fontSize: '0.95vw', fontWeight: 700, letterSpacing: '0.05em' }}>WARN</span>
                <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>{t('chat_assist 20% over budget — cost score penalized', 'chat_assist 20% מעל התקציב — ציון עלות נענש')}</span>
              </div>
            </>
          ) : (
