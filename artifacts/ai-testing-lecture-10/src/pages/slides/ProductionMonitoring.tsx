      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5vh' }}>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: '1.1vw', color: '#475569', lineHeight: 1.5 }}>{t('Sample 1–5% of live requests for shadow evaluation', 'דגום 1–5% מהבקשות החיות להערכת צל')}</div>
          </div>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', flexShrink: 0, marginTop: '0.5vw' }} />
            <div style={{ fontSize: '1.1vw', color: '#475569', lineHeight: 1.5 }}>{t('Alert when rolling 24h accuracy drops 5% below the nightly baseline', 'התראה כאשר הדיוק הנגלל של 24 שעות יורד 5% מקו הבסיס הלילי')}</div>
          </div>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'flex-start' }}>
            <div style={{ width: '0.6vw', height: '0.6vw', borderRadius: '50%', backgroundColor: '#0D9488', flexShrink: 0, marginTop: '0.5vw' }} />
            <div style={{ fontSize: '1.1vw', color: '#475569', lineHeight: 1.5 }}>{t('Store sampled evals to Supabase for trend analysis and manual review', 'אחסן הערכות דגומות ב-Supabase לניתוח מגמות ובדיקה ידנית')}</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ background: '#1E3A5F', padding: '4vh 3vw', borderRadius: '1vw', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '2.5vh' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,0.8)' }}>{t('Shadow eval pass rate', 'שיעור הצלחת הערכת צל')}</div>
              <div style={{ fontSize: '1.1vw', fontWeight: 700, color: '#0D9488' }}>&gt; 90%</div>
            </div>
            <div style={{ height: '1px', background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,0.8)' }}>{t('p95 latency (live)', 'זמן אחזור p95 (חי)')}</div>
              <div style={{ fontSize: '1.1vw', fontWeight: 700, color: '#0D9488' }}>&lt; 4s</div>
            </div>
            <div style={{ height: '1px', background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,0.8)' }}>{t('Error rate', 'שיעור שגיאות')}</div>
              <div style={{ fontSize: '1.1vw', fontWeight: 700, color: '#0D9488' }}>&lt; 1%</div>
            </div>
            <div style={{ height: '1px', background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,0.8)' }}>{t('Cost / 1k requests', 'עלות / 1k בקשות')}</div>
              <div style={{ fontSize: '1.1vw', fontWeight: 700, color: '#0D9488' }}>{t('vs. budget', 'מול תקציב')}</div>
            </div>
          </div>
        </div>
      </div>
