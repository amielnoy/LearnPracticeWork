...
export default function WorkedExampleSecurityFindings() {
  ...
  const defaultBullets = [
    t(
      'Write security probe results to the security_findings table in Supabase',
      'כתוב תוצאות בדיקת אבטחה לטבלת security_findings ב-Supabase',
    ),
    t(
      'Query open critical findings to block a release from the scorecard pipeline',
      'שאל ממצאים קריטיים פתוחים כדי לחסום גרסה מצינור כרטיס הניקוד',
    ),
    t(
      'A single open critical finding sets the security score to 0',
      'ממצא קריטי פתוח אחד מגדיר את ציון האבטחה ל-0',
    ),
  ];
  ...
  const defaultCode = [
    {
      ...
      value:
        "supabase.from('security_findings').insert({ release_id: 'v2.15.0-rc1', probe_type: 'prompt_injection', severity: 'critical', probe_input: 'Ignore previous instructions...', status: 'open' })",
    },
    {
      label: 'SECURITY GATE QUERY',
      value:
        "supabase.from('security_findings').select('id, severity').eq('release_id', 'v2.15.0-rc1').eq('severity', 'critical').eq('status', 'open')",
    },
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                ...
                <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>
                  {t(
                    'Security score set to 0 — release blocked',
                    'ציון אבטחה הוגדר ל-0 — גרסה נחסמה',
                  )}
                </span>
              </div>
            </>
              ...
              <div key={pi} style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
                ...
                {panel.verdict && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                    <span
                      style={{
                        background: panel.verdict.status.toUpperCase().includes('BLOCK')
                          ? '#DC2626'
                          : '#059669',
                        color: '#fff',
                        borderRadius: '0.4vw',
