      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5vh' }}>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>{t('LLM-judge pass rate', 'שיעור הצלחת שופט LLM')}</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: > 90%', 'יעד: > 90%')}</div>
          </div>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>{t('Golden-set mean score', 'ציון ממוצע ערכה זהובה')}</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: >= baseline', 'יעד: >= קו בסיס')}</div>
          </div>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>{t('Hallucination rate', 'שיעור הזיות')}</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: < 2%', 'יעד: < 2%')}</div>
          </div>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>{t('Refusal rate on valid inputs', 'שיעור דחייה על קלטים תקינים')}</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: < 1%', 'יעד: < 1%')}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ background: '#FFFFFF', padding: '4vh 3vw', borderRadius: '1vw', border: '1px solid #E2E8F0', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '2.5vh', boxShadow: '0 0.5vw 1.5vw rgba(30,58,95,0.05)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh', textAlign: isHe ? 'right' : 'left' }}>
            <div>
              <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F', marginBottom: '0.5vh' }}>{t('Start from current reality', 'התחל מהמציאות הנוכחית')}</div>
              <div style={{ fontSize: '1vw', color: '#64748B', lineHeight: 1.5 }}>{t('Measure your baseline first. Aspirational targets that your current model cannot meet block every deploy.', 'מדוד קודם את קו הבסיס שלך. יעדים שאפתניים שהמודל הנוכחי שלך לא יכול לעמוד בהם חוסמים כל פריסה.')}</div>
            </div>
            <div>
              <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F', marginBottom: '0.5vh' }}>{t('Use relative deltas, not absolutes', 'השתמש בשינויים יחסיים, לא מוחלטים')}</div>
              <div style={{ fontSize: '1vw', color: '#64748B', lineHeight: 1.5 }}>{t('"Score must not drop more than 5% from the last release baseline" is more robust than "score must be > 85".', '"הציון לא יכול לרדת יותר מ-5% מקו הבסיס של הגרסה האחרונה" חזק יותר מ"ציון חייב להיות > 85".')}</div>
            </div>
          </div>
        </div>
      </div>
