      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5vh' }}>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: < 2s', 'יעד: < 2 שניות')}</div>
          </div>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>p95</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: < 4s', 'יעד: < 4 שניות')}</div>
          </div>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>p99</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: < 8s', 'יעד: < 8 שניות')}</div>
          </div>
          <div style={{ background: '#FFFFFF', padding: '1.8vh 1.5vw', borderRadius: '0.8vw', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F' }}>{t('TTFT (time-to-first-token)', 'TTFT (זמן לאסימון ראשון)')}</div>
            <div style={{ fontSize: '1vw', color: '#0D9488', fontWeight: 600 }}>{t('Target: < 800ms', 'יעד: < 800ms')}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ background: '#FFFFFF', padding: '4vh 3vw', borderRadius: '1vw', border: '1px solid #E2E8F0', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '2.5vh', boxShadow: '0 0.5vw 1.5vw rgba(30,58,95,0.05)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh', textAlign: isHe ? 'right' : 'left' }}>
            <div style={{ background: '#F8FAFC', borderRadius: '0.8vw', padding: '2vh 2vw', fontFamily: "'SFMono-Regular', monospace", fontSize: '1.1vw', color: '#1E3A5F' }}>
              <div>score = 100</div>
            ...
            <div style={{ fontSize: '1vw', color: '#64748B', lineHeight: 1.5 }}>
              {t('The latency score feeds directly into the overall quality scorecard. A fast-but-inaccurate model and a slow-but-accurate model both lose points.', 'ציון זמן האחזור מוזן ישירות לכרטיס ניקוד האיכות הכולל. מודל מהיר אך לא מדויק ומודל איטי אך מדויק שניהם מפסידים נקודות.')}
            </div>
          </div>
        </div>
      </div>
