...
export default function WorkedExampleRolloutReport() {
  ...
  const defaultBullets = [
    t(
      'Query Supabase to build a rollout progress report across all teams',
      'שאל את Supabase כדי לבנות דוח התקדמות גלגול בכל הצוותים',
    ),
    t(
      'Each row tracks which testing layers a team has adopted',
      'כל שורה עוקבת אחר שכבות הבדיקה שצוות אימץ',
    ),
    t(
      'Filter for teams that have not yet adopted integration testing',
      'סנן לצוותים שעדיין לא אימצו בדיקות אינטגרציה',
    ),
  ];
  ...
  const defaultCode = [
    { label: 'TABLE', value: 'team_rollout_progress' },
    {
      label: 'QUERY',
      value:
        "supabase.from('team_rollout_progress').select('team_name, has_unit_tests, has_integration_tests, has_system_tests, has_prod_monitoring').eq('has_integration_tests', false).order('team_name')",
    },
    {
      label: 'RETURNS',
      value:
        "[ { team_name: 'checkout', has_unit_tests: true, has_integration_tests: false, ... }, { team_name: 'search', has_unit_tests: true, has_integration_tests: false, ... } ]",
    },
  ];
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1vw' }}>
                ...
                <span style={{ fontSize: '0.95vw', color: '#94A3B8' }}>
                  {t('Need integration-test onboarding', 'זקוקים להכשרה בבדיקות אינטגרציה')}
                </span>
              </div>
            </>
          ) : (
