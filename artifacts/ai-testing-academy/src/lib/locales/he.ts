...
export const he: Locale = {
  ...
  setup: {
    ...
    lead: 'האקדמיה פועלת עם Groq כברירת מחדל חינמית ומהירה — אין צורך בהגדרה. אפשר לחבר בכל עת מפתח Claude או OpenAI משלכם למודל אחר.',
    ...
    notice:
      'המפתח שלך נשמר רק למשך סשן הדפדפן, אלא אם תבחר במפורש לזכור אותו, ונשלח ישירות ל-API של הספק. בשימוש במפתח ברירת המחדל, הבקשות עוברות דרך השרת — המפתח עצמו לעולם לא מגיע לדפדפן. העשרת החיפוש החי בספריית התרגול תמיד משתמשת בחיפוש Google בנפרד מהגדרה זו.',
    providers: [
      { value: 'groq', label: 'Groq (ברירת מחדל חינמית)' },
  ...
  s: {
    ...
    errGroqKeyHint: ' (מפתחות Groq מתחילים ב-gsk_ — קבל בחינם ב-console.groq.com/keys)',
