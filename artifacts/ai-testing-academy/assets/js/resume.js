...
export async function handleResumeFile(file) {
    if (!file)
        return;
    ...
    try {
        ...
        if (ext === 'pdf')
            ...
            throw new Error(S.errFormatPrefix + ext + S.errFormatSuffix);
        text = text.replace(/[ \t]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim();
        if (text.length < 40)
            throw new Error(S.errExtractFail);
...
export async function evaluateResume() {
    ...
    $('resumeErr').textContent = '';
    if (txt.length < 80) {
        $('resumeErr').textContent = S.errResumeEmpty;
        return;
    }
    const btn = $('resumeBtn');
    btn.disabled = true;
    btn.innerHTML = S.btnEvaluating;
    try {
        const role = $('targetRole').value.trim() || 'QA Automation Engineer';
        const reply = await callClaude(RESUME_SYSTEM, [{ role: 'user', content: S.promptRolePrefix + role + S.promptResumeLabel + txt }]);
       ...
       <div class="bar"><div style="width:${pct}%"></div></div></div>`;
        ...
        fill('resumeStrengths', r.strengths);
        fill('resumeGaps', r.gaps);
        fill('resumeRecs', r.recommendations);
...
export async function showImprovedResume() {
    ...
    try {
        ...
        $('improvedErr').textContent = e.message;
    }
    btn.disabled = false;
    btn.textContent = S.btnBuildResume;
}
...
export async function downloadImprovedPdf() {
    const btn = $('pdfBtn');
    $('improvedErr').textContent = '';
    btn.disabled = true;
    btn.textContent = S.btnPreparingPdf;
    try {
        const text = await ensureImprovedResume();
    ...
    btn.disabled = false;
    btn.textContent = S.btnDownloadPdf;
}
...
export function initResume() {
    ...
    if ($('jobDesc'))
        $('jobDesc').addEventListener('input', () => { improvedResume = null; });
