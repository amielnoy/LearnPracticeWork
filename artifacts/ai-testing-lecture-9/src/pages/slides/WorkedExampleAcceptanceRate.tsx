...
const FALLBACK_ROWS = [
  { label: 'QUERY', value: "SELECT sprint_week, COUNT(*) FILTER (WHERE review_status='approved') / COUNT(*)::float AS acceptance_rate FROM generated_tests GROUP BY sprint_week ORDER BY sprint_week" },
  { label: 'RESULT', value: "week=1: 0.62  week=2: 0.71  week=3: 0.78" },
  { label: 'INSIGHT', value: "Acceptance rate improved 16pp as team refined prompt patterns over 3 sprints" },
];
...
export default function WorkedExampleAcceptanceRate() {
  ...
  const bullets = example?.bullets ?? [
    t('Query generated_tests grouped by sprint_week to compute acceptance rate over time', 'שאילתת generated_tests מקובצת לפי sprint_week לחישוב שיעור קבלה לאורך זמן'),
    t('Acceptance rate = approved rows / total rows for that sprint', 'שיעור קבלה = שורות מאושרות / סה"כ שורות עבור אותו ספרינט'),
    t('Declining rate signals prompt quality issues or reviewer fatigue', 'ירידה בשיעור מסמנת בעיות באיכות הפרומפט או עייפות של הסוקרים'),
  ];
    ...
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <h1 style={{ fontSize: '2.4vw', fontWeight: 800, margin: '0 0 3vh 0', lineHeight: 1.15, letterSpacing: '-0.02em' }}>
          {example?.title ?? t('Querying Acceptance Rate from Supabase', 'שאילתת שיעור קבלה מ-Supabase')}
        </h1>
        ...
        <div style={codePanel}>
          {failed || !example ? (
            <>
              ...
              <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#7DD3FC' }}>{'// Supabase: acceptance rate over time'}</div>
              <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0' }}>{'const { data } = await supabase.rpc('}</div>
              <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0', paddingLeft: '2vw' }}>{" 'acceptance_rate_by_sprint'"}</div>
