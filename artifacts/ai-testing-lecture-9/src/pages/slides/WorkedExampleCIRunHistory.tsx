...
const FALLBACK_ROWS = [
  { label: 'TABLE', value: 'ci_run_history' },
  {
    label: 'INSERT',
    value:
      "{ run_id: 'ci-882', pr_number: 441, total: 214, passed: 209, failed: 5, duration_ms: 47200 }",
  },
  {
    label: 'TREND',
    value: "SELECT DATE_TRUNC('week', created_at), AVG(failed) FROM ci_run_history GROUP BY 1",
  },
...
export default function WorkedExampleCIRunHistory() {
  ...
  const bullets = example?.bullets ?? [
    t(
      'Record every CI run: run_id, PR number, total tests, passed, failed, and duration in ms',
      'תעד כל ריצת CI: run_id, מספר PR, סך בדיקות, עברו, נכשלו, ומשך זמן ב-ms',
    ),
    t(
      'Weekly averages of failed-test count reveal whether the generated test suite is getting more stable over time',
      'ממוצעים שבועיים של מספר בדיקות שנכשלו מגלים אם חבילת הבדיקות שנוצרה נהיית יציבה יותר עם הזמן',
    ),
    t(
      'Supabase Edge Function fires a Slack alert when failed count exceeds threshold',
      'פונקציית Supabase Edge שולחת התראת Slack כאשר מספר הכשלים חורג מסף',
    ),
  ];
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codePanel}>
          ...
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#7DD3FC' }}>
            {'// Record CI run after each pipeline execution'}
          </div>
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0' }}>
            {"await supabase.from('ci_run_history').insert({"}
          </div>
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0', paddingLeft: '2vw' }}>
            {' '}
            {'run_id, pr_number, total_tests,'}
          </div>
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0', paddingLeft: '2vw' }}>
            {' '}
            {'passed_tests, failed_tests, duration_ms'}
          </div>
