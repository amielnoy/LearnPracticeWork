...
const FALLBACK_ROWS = [
  ...
  { label: 'INSERT', value: "{ run_date, tool: 'copilot', prompt_version: 'v2.1', generated: 12, accepted: 9, flaky: 2 }" },
  { label: 'QUERY', value: "SELECT tool, AVG(accepted::float/generated) AS accept_rate FROM generator_runs GROUP BY tool" },
...
export default function WorkedExampleGeneratorComparison() {
  ...
  const bullets = example?.bullets ?? [
    t('Track each generation run per tool in generator_runs: date, tool, prompt version, generated count, accepted count', 'עקוב אחר כל ריצת יצירה לכל כלי ב-generator_runs: תאריך, כלי, גרסת פרומפט, מספר שנוצרו, מספר שאושרו'),
    t('Aggregate acceptance rate per tool to compare Copilot, Cursor, and custom pipeline quality', 'צבור שיעור קבלה לכל כלי להשוואת איכות Copilot, Cursor וצינור מותאם'),
    t('Prompt version column enables before/after analysis when prompts are changed', 'עמודת גרסת הפרומפט מאפשרת ניתוח לפני/אחרי בעת שינוי פרומפטים'),
  ];
    ...
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codePanel}>
          ...
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#7DD3FC' }}>{'// Insert run metadata per generation session'}</div>
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0' }}>{"await supabase.from('generator_runs').insert({"}</div>
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0', paddingLeft: '2vw' }}>  {'run_date, tool, prompt_version,'}</div>
          <div style={{ fontSize: '1vw', lineHeight: 1.7, color: '#E2E8F0', paddingLeft: '2vw' }}>  {'generated_count, accepted_count, flaky_count'}</div>
