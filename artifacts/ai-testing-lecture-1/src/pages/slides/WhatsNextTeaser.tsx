    ...
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '4.6vw', fontWeight: 800, margin: '0 0 2vh 0', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
          {t("What\u2019s Next: Lecture 2", 'מה הלאה: הרצאה 2')}
        </h1>
        <p style={{ fontSize: '1.4vw', fontWeight: 400, color: '#475569', margin: '0 0 6vh 0', lineHeight: 1.5, maxWidth: '50vw' }}>
          {t(
            ...
            'נבנה מאפס מערך נתונים זהב ונחבר את הרצת ההערכה האוטומטית הראשונה שלכם.'
          )}
        </p>
        ...
        <div style={{ display: 'flex', gap: '4vw', padding: '4vh 6vw', background: '#FFFFFF', borderRadius: '1vw', border: '1px solid #E2E8F0', boxShadow: '0 0.5vw 1.5vw rgba(30, 58, 95, 0.05)' }}>
          <div style={{ textAlign: isHe ? 'right' : 'left' }}>
            <div style={{ fontSize: '0.9vw', fontWeight: 600, color: '#64748B', marginBottom: '0.5vh', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Track', 'מסלול')}</div>
            <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F' }}>{t('AI Testing', 'בדיקות AI')}</div>
          </div>
          <div style={{ width: '1px', backgroundColor: '#E2E8F0' }} />
          <div style={{ textAlign: isHe ? 'right' : 'left' }}>
            <div style={{ fontSize: '0.9vw', fontWeight: 600, color: '#64748B', marginBottom: '0.5vh', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Next Lecture', 'הרצאה הבאה')}</div>
            <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F' }}>{t('Golden Datasets in Practice', 'מערכי נתונים זהב בפועל')}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #E2E8F0', paddingTop: '2vh', fontSize: '0.9vw', color: '#94A3B8', fontWeight: 500 }}>
        ...
        <div style={{ display: 'flex', gap: '1vw' }}>
          ...
          <span>&bull;</span>
          <span>{t('Slide 19 of 19', 'שקופית 19 מתוך 19')}</span>
        </div>
      </div>
    </div>
  );
