...
const ROW_COLORS = ['#38BDF8', '#94A3B8', '#FBBF24'];

export default function WorkedExampleHallucination() {
  const [example, setExample] = useState<LectureExample | null>(null);
  const [failed, setFailed] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetchLectureExample(16)
      .then(data => {
    ...
    <div style={wrap} dir={dir}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '1vw' }}>
          ...
          <span>&bull;</span>
          <span>{t('Slide 16 of 22', 'שקופית 16 מתוך 22')}</span>
        </div>
      </div>
    </div>
  );
