  ...
  return (
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
        <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#0D9488', marginBottom: '2vh', textTransform: isHe ? 'none' : 'uppercase', letterSpacing: '0.05em' }}>{t('Coming in the Security Track', 'בקרוב במסלול האבטחה')}</div>
        ...
        <div style={{ display: 'flex', gap: '4vw', padding: '4vh 6vw', background: '#FFFFFF', borderRadius: '1vw', border: '1px solid #E2E8F0', boxShadow: '0 0.5vw 1.5vw rgba(30, 58, 95, 0.05)' }}>
          <div style={{ textAlign: isHe ? 'right' : 'left' }}>
            <div style={{ fontSize: '0.9vw', fontWeight: 600, color: '#64748B', marginBottom: '0.5vh', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Covered In', 'מכוסה ב')}</div>
            <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F' }}>{t('AI-Powered Basic Cybersecurity', 'אבטחת סייבר בסיסית מבוססת AI')}</div>
          </div>
          <div style={{ width: '1px', backgroundColor: '#E2E8F0' }} />
          <div style={{ textAlign: isHe ? 'right' : 'left' }}>
            <div style={{ fontSize: '0.9vw', fontWeight: 600, color: '#64748B', marginBottom: '0.5vh', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Starts At', 'מתחיל ב')}</div>
            <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F' }}>{t('Lecture 7', 'הרצאה 7')}</div>
          </div>
        </div>
      </div>
