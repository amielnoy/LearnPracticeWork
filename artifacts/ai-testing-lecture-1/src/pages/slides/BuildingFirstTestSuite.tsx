  ...
  return (
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#0D9488', marginBottom: '1vh', textTransform: isHe ? 'none' : 'uppercase', letterSpacing: '0.05em' }}>{t('Hands-On', 'עבודה מעשית')}</div>
        <h1 style={{ fontSize: '3.4vw', fontWeight: 800, margin: '0 0 2vh 0', lineHeight: 1.1, letterSpacing: '-0.02em' }}>{t('Building Your First AI Test Suite', 'בניית חבילת הבדיקות הראשונה שלך ל-AI')}</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
          <div style={card}>
            ...
            <div>
              <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F', marginBottom: '0.5vh' }}>{t('Define Expected Behavior', 'הגדרת התנהגות צפויה')}</div>
              <div style={{ fontSize: '1vw', color: '#64748B', lineHeight: 1.4 }}>{t('Write down what a \u201cgood\u201d answer looks like before automating anything.', 'רשמו כיצד נראית תשובה "טובה" לפני שמבצעים כל אוטומציה.')}</div>
            </div>
          </div>
          <div style={card}>
            <div style={badge}>2</div>
            <div>
              <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F', marginBottom: '0.5vh' }}>{t('Assemble a Small Golden Set', 'הרכבת סט זהב קטן')}</div>
              <div style={{ fontSize: '1vw', color: '#64748B', lineHeight: 1.4 }}>{t('Ten to twenty real examples beat a hundred synthetic ones.', 'עשרה עד עשרים דוגמאות אמיתיות עדיפות על מאה דוגמאות סינתטיות.')}</div>
            </div>
          </div>
          <div style={card}>
            <div style={badge}>3</div>
            <div>
              <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#1E3A5F', marginBottom: '0.5vh' }}>{t('Automate the First Pass', 'אוטומציה של הסבב הראשון')}</div>
              <div style={{ fontSize: '1vw', color: '#64748B', lineHeight: 1.4 }}>{t('Wire up similarity or rule-based checks before reaching for an LLM judge.', 'חברו בדיקות מבוססות דמיון או כללים לפני שפונים ל-LLM כשופט.')}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ background: '#FFFFFF', padding: '4vh 3vw', borderRadius: '1vw', border: '1px solid #E2E8F0', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '3vh', boxSizing: 'border-box', boxShadow: '0 0.5vw 1.5vw rgba(30, 58, 95, 0.05)' }}>
          <div style={{ fontSize: '1.5vw', fontWeight: 700, color: '#1E3A5F', borderBottom: '1px solid #E2E8F0', paddingBottom: '2vh', textAlign: isHe ? 'right' : 'left' }}>{t('Then Keep Going', 'ואז ממשיכים')}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3vh', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: '2vw', alignItems: 'center', position: 'relative', zIndex: 1 }}>
              ...
              <div style={{ fontSize: '1.15vw', fontWeight: 500, color: '#1E3A5F' }}>{t('Add LLM-as-judge for nuance', 'הוסיפו LLM כשופט לניואנסים')}</div>
            </div>
            <div style={{ display: 'flex', gap: '2vw', alignItems: 'center', position: 'relative', zIndex: 1 }}>
              <div style={{ width: '1vw', height: '1vw', backgroundColor: '#0D9488', borderRadius: '50%', border: '4px solid #FFFFFF', boxShadow: '0 0 0 1px #E2E8F0' }} />
              <div style={{ fontSize: '1.15vw', fontWeight: 500, color: '#1E3A5F' }}>{t('Wire results into CI', 'חברו תוצאות ל-CI')}</div>
            </div>
            <div style={{ display: 'flex', gap: '2vw', alignItems: 'center', position: 'relative', zIndex: 1 }}>
              <div style={{ width: '1vw', height: '1vw', backgroundColor: '#0D9488', borderRadius: '50%', border: '4px solid #FFFFFF', boxShadow: '0 0 0 1px #E2E8F0' }} />
              <div style={{ fontSize: '1.15vw', fontWeight: 500, color: '#1E3A5F' }}>{t('Review failures weekly, not just at launch', 'סקרו כשלים מדי שבוע, לא רק בהשקה')}</div>
            </div>
          </div>
        </div>
      </div>
