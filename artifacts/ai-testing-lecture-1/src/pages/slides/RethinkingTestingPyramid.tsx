  ...
  return (
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#0D9488', marginBottom: '1vh', textTransform: isHe ? 'none' : 'uppercase', letterSpacing: '0.05em' }}>{t('A New Shape', 'צורה חדשה')}</div>
        <h1 style={{ fontSize: '3.6vw', fontWeight: 800, margin: '0 0 2vh 0', lineHeight: 1.1, letterSpacing: '-0.02em' }}>{t('Rethinking the Testing Pyramid', 'חשיבה מחודשת על פירמידת הבדיקות')}</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '1vh' }}>
        <div style={{ width: '22vw', padding: '2vh 1.5vw', backgroundColor: '#0D9488', color: '#FFFFFF', borderRadius: '0.6vw', textAlign: 'center', boxShadow: '0 0.5vw 1.5vw rgba(30, 58, 95, 0.1)' }}>
          <div style={{ fontSize: '1.1vw', fontWeight: 700 }}>{t('Human Review', 'סקירה אנושית')}</div>
          <div style={{ fontSize: '0.85vw', marginTop: '0.6vh', opacity: 0.9 }}>{t('Spot checks on high-stakes or novel cases', 'בדיקות מדגמיות על מקרים בעלי סיכון גבוה או חדשניים')}</div>
        </div>
        <div style={{ width: '30vw', padding: '2.2vh 1.5vw', backgroundColor: 'rgba(13, 148, 136, 0.55)', color: '#FFFFFF', borderRadius: '0.6vw', textAlign: 'center', boxShadow: '0 0.5vw 1.5vw rgba(30, 58, 95, 0.08)' }}>
          <div style={{ fontSize: '1.15vw', fontWeight: 700 }}>{t('Evaluated Behavior', 'התנהגות מוערכת')}</div>
          <div style={{ fontSize: '0.9vw', marginTop: '0.6vh' }}>{t('Golden sets, LLM-as-judge, similarity scoring', 'מערכי נתוני ייחוס, LLM-as-judge, ניקוד דמיון')}</div>
        </div>
        <div style={{ width: '38vw', padding: '2.4vh 1.5vw', backgroundColor: 'rgba(13, 148, 136, 0.18)', color: '#1E3A5F', borderRadius: '0.6vw', textAlign: 'center', border: '1px solid #E2E8F0' }}>
          <div style={{ fontSize: '1.2vw', fontWeight: 700 }}>{t('Deterministic Checks', 'בדיקות דטרמיניסטיות')}</div>
          <div style={{ fontSize: '0.95vw', marginTop: '0.6vh', color: '#475569' }}>{t('Schema, format, and safety-filter validation', 'אימות סכמה, פורמט ומסנני בטיחות')}</div>
        </div>
      </div>
