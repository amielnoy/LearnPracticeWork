  ...
  return (
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', gap: '4vh' }}>
        <div>
          <div style={{ fontSize: '1.2vw', fontWeight: 600, color: '#0D9488', marginBottom: '2vh', textTransform: isHe ? 'none' : 'uppercase', letterSpacing: '0.05em' }}>{t('The Core Challenge', 'האתגר המרכזי')}</div>
          <h1 style={{ fontSize: '3.6vw', fontWeight: 800, margin: '0 0 2.5vh 0', lineHeight: 1.2, letterSpacing: '-0.02em', maxWidth: '60vw' }}>
            {t('Ask the same question twice. Get two different, both-defensible answers.', 'שאל את אותה שאלה פעמיים. קבל שתי תשובות שונות — שתיהן ניתנות להצדקה.')}
          </h1>
        ...
        <div style={{ display: 'flex', gap: '2vw', width: '70vw' }}>
          <div style={pair}>
            <div style={{ fontSize: '1vw', fontWeight: 600, color: '#64748B', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Same Prompt', 'אותו פרומפט')}</div>
            <div style={{ fontSize: '1.6vw', fontWeight: 700, color: '#1E3A5F', margin: '1vh 0' }}>&rarr;</div>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#0D9488' }}>{t('Different Output', 'פלט שונה')}</div>
          </div>
          <div style={pair}>
            <div style={{ fontSize: '1vw', fontWeight: 600, color: '#64748B', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Same Model', 'אותו מודל')}</div>
            <div style={{ fontSize: '1.6vw', fontWeight: 700, color: '#1E3A5F', margin: '1vh 0' }}>&rarr;</div>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#0D9488' }}>{t('Different Day', 'יום שונה')}</div>
          </div>
          <div style={pair}>
            <div style={{ fontSize: '1vw', fontWeight: 600, color: '#64748B', textTransform: isHe ? 'none' : 'uppercase' }}>{t('Same Data', 'אותם נתונים')}</div>
            <div style={{ fontSize: '1.6vw', fontWeight: 700, color: '#1E3A5F', margin: '1vh 0' }}>&rarr;</div>
            <div style={{ fontSize: '1.1vw', fontWeight: 600, color: '#0D9488' }}>{t('Different Ranking', 'דירוג שונה')}</div>
          </div>
        </div>
      </div>
