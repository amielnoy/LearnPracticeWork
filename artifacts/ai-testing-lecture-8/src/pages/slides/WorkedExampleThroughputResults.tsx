...
export default function WorkedExampleThroughputResults() {
  return (
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <h2 style={{ fontSize: '2.4vw', fontWeight: 800, margin: '0 0 2vh 0', lineHeight: 1.15, letterSpacing: '-0.02em', color: '#F1F5F9' }}>
          {t('Store Throughput Results and Compute Degradation', 'שמור תוצאות רוחב פס וחשב דגרדציה')}
        </h2>
        <p style={{ fontSize: '1.05vw', color: 'rgba(255,255,255,0.6)', lineHeight: 1.6, margin: '0 0 2.5vh 0' }}>
          {t(
            'Write requests/sec at each concurrency level to throughput_runs table. Query to find where latency inflects and mark that as the degradation point.',
            'כתוב בקשות/שניה בכל רמת מקביליות לטבלת throughput_runs. שאל כדי למצוא היכן זמן האחזור משתנה וסמן זאת כנקודת הדגרדציה.',
          )}
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1vh' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0.6vw', padding: '1.5vh 1.5vw', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: '0.95vw', color: '#CBD5E1', fontFamily: 'monospace', lineHeight: 1.6 }}>
              concurrency, rps, p95_ms,<br />error_rate, degraded
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codeBlock}>
          <div style={{ color: '#64748B', marginBottom: '1vh', fontSize: '0.9vw' }}>// store-throughput-results.ts</div>
          ...
          <div style={{ marginTop: '1.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>async function</span> <span style={{ color: '#F472B6' }}>storeThroughputLevel</span>(data: ThroughputLevel) {'{'}
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#94A3B8' }}>// determine if this level shows degradation</span>
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>const</span> degraded = data.p95_ms {'>'} SLA_THRESHOLD_MS
          </div>
          <div style={{ paddingLeft: '4vw', marginBottom: '1vh' }}>|| data.error_rate {'>'} 0.01;</div>
          <div style={{ paddingLeft: '2vw' }}>
          ...
          <div style={{ paddingLeft: '6vw' }}>concurrency: data.concurrency,</div>
          <div style={{ paddingLeft: '6vw' }}>rps: data.requestsPerSec,</div>
          <div style={{ paddingLeft: '6vw' }}>p95_ms: data.p95_ms,</div>
          <div style={{ paddingLeft: '6vw' }}>error_rate: data.error_rate,</div>
          <div style={{ paddingLeft: '6vw' }}>degraded,</div>
          <div style={{ paddingLeft: '6vw' }}>run_at: <span style={{ color: '#F472B6' }}>new</span> Date().toISOString(),</div>
          <div style={{ paddingLeft: '4vw' }}>{'}'});</div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>if</span> (degraded) console.warn(<span style={{ color: '#86EFAC' }}>`Degradation at c=${'{'}data.concurrency{'}'}`</span>);
          </div>
          <div>{'}'}</div>
        </div>
      </div>
