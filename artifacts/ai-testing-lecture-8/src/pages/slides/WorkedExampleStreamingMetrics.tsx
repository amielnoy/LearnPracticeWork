  ...
  return (
    <div style={wrap} dir={dir}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1vh' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0.6vw', padding: '1.5vh 1.5vw', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: '0.95vw', color: '#CBD5E1', fontFamily: 'monospace', lineHeight: 1.6 }}>
              ttft_ms, total_ms,<br />input_tokens, output_tokens
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codeBlock}>
          <div style={{ color: '#64748B', marginBottom: '1vh', fontSize: '0.9vw' }}>// measure-streaming-ttft.ts</div>
          ...
          <div style={{ marginTop: '1.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>async function</span> <span style={{ color: '#F472B6' }}>measureAndStore</span>(prompt: string) {'{'}
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>const</span> t0 = performance.now();
          </div>
          <div style={{ paddingLeft: '2vw', marginTop: '0.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>const</span> stream = <span style={{ color: '#7DD3FC' }}>await</span> openai.chat.completions
          </div>
          <div style={{ paddingLeft: '4vw' }}>.create({'{ model, messages, stream: true }'});</div>
          <div style={{ paddingLeft: '2vw', marginTop: '0.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>let</span> ttft = <span style={{ color: '#FDA4AF' }}>0</span>, total = <span style={{ color: '#FDA4AF' }}>0</span>;
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>for await</span> (<span style={{ color: '#7DD3FC' }}>const</span> chunk <span style={{ color: '#7DD3FC' }}>of</span> stream) {'{'}
          </div>
          ...
          <div style={{ paddingLeft: '2vw', marginTop: '1vh' }}>
            <span style={{ color: '#7DD3FC' }}>await</span> sb.from(<span style={{ color: '#86EFAC' }}>'perf_runs'</span>).insert({'{'}</div>
          <div style={{ paddingLeft: '4vw' }}>ttft_ms: Math.round(ttft),</div>
          <div style={{ paddingLeft: '4vw' }}>total_ms: Math.round(total),</div>
          <div style={{ paddingLeft: '4vw' }}>run_at: <span style={{ color: '#F472B6' }}>new</span> Date().toISOString(),</div>
          <div style={{ paddingLeft: '2vw' }}>{'}'});</div>
          <div>{'}'}</div>
        </div>
      </div>
