  ...
  return (
    <div style={wrap} dir={dir}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1vh' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0.6vw', padding: '1.5vh 1.5vw', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: '0.95vw', color: '#CBD5E1', fontFamily: 'monospace', lineHeight: 1.6 }}>
              feature, cost_usd,<br />input_tokens, output_tokens, ts
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codeBlock}>
          <div style={{ color: '#64748B', marginBottom: '1vh', fontSize: '0.9vw' }}>// detect-cost-regression.ts</div>
          ...
          <div style={{ marginTop: '1.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>async function</span> <span style={{ color: '#F472B6' }}>checkCostRegression</span>(feature: string) {'{'}
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#94A3B8' }}>// 30-day rolling average</span>
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>const</span> since = <span style={{ color: '#F472B6' }}>new</span> Date(
          </div>
          <div style={{ paddingLeft: '4vw' }}>Date.now() - <span style={{ color: '#FDA4AF' }}>30</span> * <span style={{ color: '#FDA4AF' }}>86400000</span>
          </div>
          ...
          <div style={{ paddingLeft: '4vw' }}>.from(<span style={{ color: '#86EFAC' }}>'request_costs'</span>)</div>
          <div style={{ paddingLeft: '4vw' }}>.select(<span style={{ color: '#86EFAC' }}>'cost_usd'</span>)</div>
          <div style={{ paddingLeft: '4vw' }}>.eq(<span style={{ color: '#86EFAC' }}>'feature'</span>, feature)</div>
          <div style={{ paddingLeft: '4vw' }}>.gte(<span style={{ color: '#86EFAC' }}>'ts'</span>, since);</div>
          <div style={{ paddingLeft: '2vw', marginTop: '0.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>const</span> avg = mean(data?.map(r =&gt; r.cost_usd) ?? []);
          </div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>if</span> (todayCost &gt; avg * <span style={{ color: '#FDA4AF' }}>1.2</span>)
          </div>
          <div style={{ paddingLeft: '4vw' }}>
            <span style={{ color: '#7DD3FC' }}>throw new</span> Error(<span style={{ color: '#86EFAC' }}>`Cost regression: ${'{'}feature{'}'}`</span>);
          </div>
          <div>{'}'}</div>
        </div>
      </div>
