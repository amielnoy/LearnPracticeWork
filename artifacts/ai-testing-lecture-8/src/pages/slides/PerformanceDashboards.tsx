    ...
    <div style={wrap} dir={dir}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2vh' }}>
          <div
            style={{ fontSize: '1.1vw', fontWeight: 600, color: '#1E3A5F', marginBottom: '0.5vh' }}
          >
            {t('Essential charts:', 'תרשימים חיוניים:')}
          </div>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '1.05vw', color: '#475569' }}>
              {t(
                'p50/p95/p99 latency over 30 days (time series)',
                'זמן אחזור p50/p95/p99 על 30 יום (סדרה זמנית)',
              )}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '1.05vw', color: '#475569' }}>
              {t(
                'Daily cost per feature (bar chart with budget line)',
                'עלות יומית לכל תכונה (תרשים עמודות עם קו תקציב)',
              )}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'center' }}>
            ...
            />
            <div style={{ fontSize: '1.05vw', color: '#475569' }}>
              {t('Error rate by model (line chart)', 'שיעור שגיאות לפי מודל (תרשים קו)')}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1vw', alignItems: 'center' }}>
            <div
            ...
            <div style={{ fontSize: '1.05vw', color: '#475569' }}>
              {t(
                'Throughput vs. baseline table (current CI run)',
                'רוחב פס לעומת טבלת בסיס (ריצת CI נוכחית)',
              )}
            </div>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          >
            {t('Mock Dashboard — Latency Trend', 'דשבורד לדוגמה — מגמת זמן אחזור')}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5vh' }}>
            ...
            >
              <div style={{ fontSize: '0.9vw', color: '#64748B' }}>
                {t('p99 latency by week', 'זמן אחזור p99 לפי שבוע')}
              </div>
              <div style={{ fontSize: '0.9vw', color: '#DC2626', fontWeight: 700 }}>
                {t('Regression at W5', 'רגרסיה ב-W5')}
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <div style={{ fontSize: '1vw', color: '#1E3A5F', lineHeight: 1.5 }}>
            {t(
              'Supabase + any charting library (Recharts, Chart.js) makes this dashboard buildable in an afternoon.',
              'Supabase + כל ספריית תרשימים (Recharts, Chart.js) הופכת את הדשבורד הזה לניתן לבנייה בצהריים.',
            )}
          </div>
        </div>
      </div>
