    ...
    <div style={wrap} dir={dir}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: isHe ? 'right' : 'left' }}>
        ...
        <h2 style={{ fontSize: '2.4vw', fontWeight: 800, margin: '0 0 2vh 0', lineHeight: 1.15, letterSpacing: '-0.02em', color: '#F1F5F9' }}>
          {t('Fetch Baseline from Supabase and Fail CI on Regression', 'שלוף בסיס מ-Supabase וכשול CI על רגרסיה')}
        </h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1vh' }}>
          <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0.6vw', padding: '1.5vh 1.5vw', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: '0.95vw', color: '#CBD5E1', fontFamily: 'monospace' }}>perf_baselines</div>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0.6vw', padding: '1.5vh 1.5vw', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: '0.95vw', color: '#CBD5E1', fontFamily: 'monospace', lineHeight: 1.6 }}>
              feature, model, p99_ms,<br />cost_usd, git_sha, created_at
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={codeBlock}>
          <div style={{ color: '#64748B', marginBottom: '1vh', fontSize: '0.9vw' }}>// ci-regression-gate.ts</div>
          ...
          <div style={{ marginTop: '1.5vh' }}>
            <span style={{ color: '#7DD3FC' }}>async function</span> <span style={{ color: '#F472B6' }}>assertNoLatencyRegression</span>(
          </div>
          ...
          <div style={{ paddingLeft: '4vw' }}>.from(<span style={{ color: '#86EFAC' }}>'perf_baselines'</span>)</div>
          <div style={{ paddingLeft: '4vw' }}>.select(<span style={{ color: '#86EFAC' }}>'p99_ms'</span>)</div>
          <div style={{ paddingLeft: '4vw' }}>.eq(<span style={{ color: '#86EFAC' }}>'feature'</span>, feature)</div>
          <div style={{ paddingLeft: '4vw' }}>.eq(<span style={{ color: '#86EFAC' }}>'model'</span>, model)</div>
          <div style={{ paddingLeft: '4vw' }}>.order(<span style={{ color: '#86EFAC' }}>'created_at'</span>, {'{ ascending: false }'})
          </div>
          <div style={{ paddingLeft: '4vw', marginBottom: '1vh' }}>.limit(1).single();</div>
          <div style={{ paddingLeft: '2vw' }}>
            <span style={{ color: '#7DD3FC' }}>const</span> threshold = data.p99_ms * <span style={{ color: '#FDA4AF' }}>1.20</span>;
          </div>
          <div style={{ paddingLeft: '2vw' }}>
          ...
          <div style={{ paddingLeft: '6vw' }}>
            <span style={{ color: '#86EFAC' }}>`p99 regression: ${'{'}currentP99Ms{'}'}ms`</span>
          </div>
