...
test('the sample CV CTA loads a useful example without asking for setup first', async ({
  hero,
  page,
}) => {
...
test('progress survives a reload and offers a continue action', async ({
  ...
  await expect(
    launcher.getByRole('link', { name: /Continue where you stopped Resume Coach/ }),
  ).toBeVisible();
  await expect(
    launcher.getByRole('progressbar', { name: 'Resume Coach — Progress' }),
